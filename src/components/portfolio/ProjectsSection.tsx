...
// Grid Projects - Modern Glassmorphism Cards
const GridProjects = ({ data, theme }: ProjectsProps) => {
  const { projects } = data;
  return (
    <section id="projects" className="py-16 sm:py-24 px-4 sm:px-6 bg-gradient-to-b from-slate-50 via-white to-slate-50">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <span className={`inline-block px-4 py-1.5 rounded-full text-xs font-semibold tracking-wider uppercase ${theme.bgLight} ${theme.primary} mb-4`}>
            Portfolio
          </span>
          <h2 className="text-3xl sm:text-5xl font-bold text-slate-900 mb-4">Featured Projects</h2>
        ...
        <ProjectsGrid>
          {projects.map((project, index) => (
            ...
            >
              <div className="group h-full bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-xl hover:shadow-slate-200/40 transition-all duration-500 overflow-hidden">
                ...
                <div className={`relative h-44 bg-gradient-to-br ${theme.gradient} overflow-hidden`}>
                  ...
                  {/* Floating tag */}
                  <span className="absolute top-4 right-4 px-3 py-1 bg-white/90 backdrop-blur-sm rounded-full text-xs font-medium text-slate-700 shadow-sm">
                    Project #{index + 1}
                  </span>
                </div>
                ...
                <div className="p-5 sm:p-6">
                  <h3 className="text-lg sm:text-xl font-bold text-slate-900 mb-2 line-clamp-1">{project.name}</h3>
                  <p className="text-slate-500 text-sm leading-relaxed mb-4 line-clamp-2">{project.description}</p>
                  
                  {/* Tech Tags */}
                  <div className="flex flex-wrap gap-2 mb-5">
                    {project.technologies?.slice(0, 3).map((tech, i) => (
                      <span key={i} className={`px-2.5 py-1 ${theme.bgLight} ${theme.primary} text-xs font-medium rounded-md`}>
                        {tech}
                      </span>
                    ))}
                  ...
                  <div className="flex gap-3 pt-4 border-t border-slate-100">
                    ...
                    {project.github && (
                      <a href={project.github} target="_blank" rel="noopener noreferrer" 
                         className="flex-1 text-center px-4 py-2.5 bg-slate-100 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-200 transition-colors">
                        Source
                      </a>
                    )}
                  </div>
    ...
    <section id="projects" className="py-16 sm:py-24 px-4 sm:px-6 bg-white">
      <div className="max-w-7xl mx-auto">
        <motion.div 
          ...
          className="text-center mb-12 sm:mb-16"
        >
          <h2 className="text-3xl sm:text-5xl font-bold text-slate-900 mb-4">My Work</h2>
          ...
          {projects.map((project, index) => (
            ...
            >
              <CardContainer containerClassName="py-0" className="w-full">
                <CardBody className="relative group/card h-full rounded-2xl p-6 bg-gradient-to-br from-slate-50 to-slate-100 border border-slate-200 hover:border-slate-300 hover:shadow-2xl transition-all duration-300">
                  {/* Gradient accent */}
                  ...
                  <CardItem translateZ={50} className="w-full">
                    <h3 className="text-lg sm:text-xl font-bold text-slate-900 line-clamp-1">{project.name}</h3>
                  </CardItem>
                  ...
                  <CardItem translateZ={30} className="w-full mt-4">
                    <div className="flex flex-wrap gap-2">
                      {project.technologies?.slice(0, 3).map((tech, i) => (
                        <span key={i} className="px-2.5 py-1 bg-white text-slate-600 text-xs font-medium rounded-md shadow-sm border border-slate-100">
                          {tech}
                        </span>
                      ))}
                    </div>
                  ...
                  <CardItem translateZ={20} className="w-full mt-5 pt-4 border-t border-slate-200">
                    <div className="flex gap-3">
                      ...
                      {project.github && (
                        <a href={project.github} target="_blank" rel="noopener noreferrer" 
                           className="flex-1 text-center px-4 py-2.5 bg-slate-200 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-300 transition-colors">
                          GitHub
                        </a>
                      )}
                    </div>
  ...
  return (
    <section id="projects" className="py-16 sm:py-24 px-4 sm:px-6 bg-white">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <h2 className="text-3xl sm:text-5xl font-bold text-slate-900 mb-4">Projects</h2>
          <p className="text-base sm:text-lg text-slate-500">Selected work I&apos;m proud of</p>
        </motion.div>
        ...
        <div className="space-y-0">
          {projects.map((project, index) => (
            ...
            >
              <div className="py-8 sm:py-10 border-b border-slate-100 hover:bg-slate-50/50 transition-colors duration-300 -mx-4 sm:-mx-6 px-4 sm:px-6">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <h3 className="text-xl sm:text-2xl font-bold text-slate-900 group-hover:text-slate-700 transition-colors mb-2">
                          {project.name}
                        </h3>
...
const FeaturedProjects = ({ data, theme }: ProjectsProps) => {
  const { projects } = data;
  return (
    <section id="projects" className="py-16 sm:py-24 px-4 sm:px-6 bg-gradient-to-b from-slate-100 via-white to-slate-50">
      <div className="max-w-7xl mx-auto">
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <h2 className="text-3xl sm:text-5xl font-bold text-slate-900 mb-4">Handpicked Projects</h2>
          <p className="text-base sm:text-lg text-slate-600 max-w-2xl mx-auto">
        ...
        <ProjectsGrid>
          {projects.map((project, index) => (
            ...
            >
              <div className="relative h-full rounded-2xl bg-white border border-slate-200 shadow-sm hover:shadow-2xl hover:shadow-slate-200/60 transition-all duration-500 overflow-hidden">
                ...
                <div className="relative">
                  <div className={`aspect-[16/10] bg-gradient-to-br ${theme.gradient} flex items-center justify-center overflow-hidden`}>
                    ...
                    {/* Floating badge */}
                    <div className="absolute top-4 left-4 px-3 py-1.5 bg-white/90 backdrop-blur-sm rounded-full shadow-sm">
                ...
                <div className="relative p-5 sm:p-6 pt-2">
                  <h3 className="text-lg sm:text-xl font-bold text-slate-900 mb-2 line-clamp-1 group-hover:text-slate-700 transition-colors">{project.name}</h3>
                  <p className="text-slate-500 text-sm leading-relaxed mb-4 line-clamp-2">{project.description}</p>
                  
                  {/* Tech Pills */}
                  <div className="flex flex-wrap gap-2 mb-5">
                    {project.technologies?.slice(0, 3).map((tech, i) => (
                      <span key={i} className={`px-3 py-1.5 ${theme.bgLight} ${theme.primary} text-xs font-medium rounded-full`}>
                        {tech}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex gap-3">
                    {project.link && (
                      <a href={project.link} target="_blank" rel="noopener noreferrer" 
                         className={`flex-1 text-center px-4 py-3 bg-gradient-to-r ${theme.gradient} text-white text-sm font-semibold rounded-xl hover:opacity-90 transition-opacity shadow-lg shadow-slate-200`}>
                        View Project
                      </a>
                    )}
                    {project.github && (
                      <a href={project.github} target="_blank" rel="noopener noreferrer" 
                         className="flex-1 text-center px-4 py-3 bg-slate-100 text-slate-700 text-sm font-semibold rounded-xl hover:bg-slate-200 transition-colors">
                        Source Code
                      </a>
                    )}
                  </div>
